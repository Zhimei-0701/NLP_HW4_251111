...
def create_prompt(sentence, k):
    ...
    prompt = (
        ...
        "Given a natural language question about the flight database, "
...
def eval_outputs(eval_x, eval_y, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    # TODO
    # No need eval_x?
    predicted_sql = eval_y

    
    save_queries_and_records(predicted_sql, model_sql_path, model_record_path)

    # compute_metrics compares ground-truth vs predict
    ...
    error_rate = error_cnt / len(model_error_msgs) if model_error_msgs else 0.0
