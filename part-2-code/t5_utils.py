...
def initialize_model(args):
    ...
    '''
    if args.finetune:
      print("Loading pretrained T5-small for finetuning...")
...
def save_model(checkpoint_dir, model, best):
    ...
    ckpt_path = os.path.join(checkpoint_dir, "best_model.pt" if best else "latest_model.pt")
    torch.save(model.state_dict(), ckpt_path)
    print(f"Model saved to {ckpt_path}")

def load_model_from_checkpoint(args, best):
    # Load model from a checkpoint
    ckpt_path = os.path.join(args.checkpoint_dir, "best_model.pt" if best else "latest_model.pt")
