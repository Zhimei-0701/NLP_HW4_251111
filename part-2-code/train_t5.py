...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
      for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader):
        encoder_input, encoder_mask = encoder_input.to(DEVICE), encoder_mask.to(DEVICE)
        decoder_input, decoder_targets = decoder_input.to(DEVICE), decoder_targets.to(DEVICE)

        logits = model(input_ids=encoder_input, attention_mask=encoder_mask, decoder_input_ids=decoder_input).logits
        non_pad = decoder_targets != PAD_IDX
        loss = loss_fn(logits[non_pad], decoder_targets[non_pad])
        total_loss += loss.item() * torch.sum(non_pad).item()
        total_tokens += torch.sum(non_pad).item()

        gen_out = model.generate(input_ids=encoder_input, attention_mask=encoder_mask,
    ...
    error_rate = len([e for e in error_msgs if e]) / len(error_msgs)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, init_decoder_input in tqdm(test_loader):
            encoder_input, encoder_mask = encoder_input.to(DEVICE), encoder_mask.to(DEVICE)
            gen_out = model.generate(input_ids=encoder_input, attention_mask=encoder_mask, max_length=256, num_beams=4)
            preds += TOKENIZER.batch_decode(gen_out, skip_special_tokens=True)
    save_queries_and_records(preds, model_sql_path, model_record_path)
...
def main():
    ...
    # ====== if exist best_model.pt，continue train ======
    # 2. Manually set checkpoint directory (same as inside train())
    model_type = 'ft' if args.finetune else 'scr'
    checkpoint_dir = os.path.join('checkpoints', f'{model_type}_experiments', args.experiment_name)
    os.makedirs(checkpoint_dir, exist_ok=True)
    args.checkpoint_dir = checkpoint_dir
    ...
    if os.path.exists(ckpt_path):
        print(f"Loading checkpoint from {ckpt_path} for continued training...")
