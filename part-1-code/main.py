...
def create_augmented_dataloader(args, dataset):
    ...
    # 1. Take the original training split (raw text)
    train_split = dataset["train"]
    # 2. Sample 5,000 random training examples (shuffle with a fixed seed for reproducibility)
    subset = train_split.shuffle(seed=42).select(range(5000))
    # 3. Apply the same custom transformation you designed in Q2
    transformed_subset = subset.map(custom_transform, load_from_cache_file=False)
    # 4. Concatenate original training data and transformed examples
    #    to build an augmented training set
    augmented_raw = datasets.concatenate_datasets([train_split, transformed_subset])
    # 5. Tokenize the augmented dataset
    augmented_tokenized = augmented_raw.map(
        tokenize_function,
        batched=True,
        load_from_cache_file=False,
    )
    # 6. Prepare for the model: remove text, rename label -> labels, set torch format
